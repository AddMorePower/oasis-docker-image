...
By default, no plugins are included in this distribution. You can find a list of available NOMAD plugins [here](https://nomad-lab.eu/prod/v1/oasis/gui/search/plugins). For a list of official plugins provided by FAIRmat, please see [here](https://github.com/AddMorePower/.github/blob/main/profile/README.md). For inspiration, you can also check the list of [plugins that are installed on the production NOMAD deployment hosted by FAIRmat](https://gitlab.mpcdf.mpg.de/nomad-lab/nomad-distro/-/raw/main/pyproject.toml?ref_type=heads).
...
When any modification is made to an AMP plugin, the Docker image of the Oasis has to be regenerated to include the modifications.
To do this:
- Go to the repository of the modified plugin and create a new tag version
- Then, in this repository, update the `pyproject.toml` file with the tag version you just created on the plugin repository
- Commit this change. The CI will automatically produce a new Docker image with the integrated modifications.

Be aware that the generation of a new Docker image can be a rather long process ~10-20 minutes.

As mentioned in the previous section, always use a commit hash or a tag version when importing a plugin from a repository.
If you only mention the link of the repository of the plugin, it might work but then, manually triggering the creation of a new Docker image might not take the lastest state of the repository for the plugin mentioned.
...
By default, documentation is built using the [nomad-docs](https://github.com/AddMorePower/nomad-docs) repository. However, if you'd like to customize the documentation for your Oasis instance, you can easily do so.

1. First, [fork the nomad-docs repository](https://github.com/AddMorePower/nomad-docs/fork).
